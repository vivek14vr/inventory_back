...
export interface INotification extends Document {
  ...
  date: string;
  /** Unique key per reminder slot: pending, before_60, admin_…, etc. */
  reminderKey: string;
  dueTime?: string;
  read: boolean;
  readAt?: Date;
...
const notificationSchema = new Schema<INotification>(
  {
    ...
    type: {
      type: String,
      enum: ["CHECKLIST_PENDING", "CHECKLIST_REMINDER", "ADMIN_REMINDER"],
      required: true,
    },
...
notificationSchema.index(
  { userId: 1, checklistId: 1, taskId: 1, date: 1, reminderKey: 1 },
  {
    ...
    partialFilterExpression: {
      type: { $in: ["CHECKLIST_PENDING", "CHECKLIST_REMINDER"] },
